...
import { BADGES, SITE, SOCIALS } from '@/lib/config';
import { getAvatarDecorationUrl, getAvatarUrl } from '@/lib/lanyard';
...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="border-border border-b py-[5rem] pb-[4.5rem] md:py-[6rem] md:pb-[5rem]"
    >
      <div className="xs:px-[1.1rem] mx-auto max-w-[740px] px-6">
        <div ref={heroRef} className="flex flex-col gap-0">
          ...
          <div className="mb-[0.6rem] flex items-center gap-4">
            ...
            <h1 className="text-text xs:text-[2.5rem] font-serif text-[clamp(2.8rem,7vw,4.5rem)] leading-[1.05] font-normal tracking-[-0.02em]">
              {SITE.name}
          ...
          <p className="text-text-muted mb-[2.5rem] max-w-[540px] text-[0.95rem] leading-[1.75]">
            {bioParts.map((part, i) =>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/contact"
              className="border-border text-text-muted hover:border-border-hover hover:bg-bg-3 hover:text-text inline-flex items-center gap-[0.4rem] rounded-[7px] border bg-transparent px-[1.1rem] py-[0.55rem] font-sans text-[0.825rem] font-medium tracking-[0.01em] no-underline transition-all duration-150"
            >
              Get in touch
            </Link>
          </div>
            ...
            {[
              ...
              return (
                <a
                  ...
                  href={social.href}
                  title={social.title}
                  className="border-border text-text-dim hover:border-border-hover hover:bg-bg-3 hover:text-text inline-flex h-[34px] w-[34px] items-center justify-center rounded-[7px] border font-mono text-[0.75rem] no-underline transition-all duration-150"
                >
